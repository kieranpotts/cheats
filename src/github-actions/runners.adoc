...
* *Self-hosted runners*: Machines that you manage, allowing you to customize the
  hardware, operating system, and network configurations.
...
offered by GitHub-hosted runners. The only requirement of the self-hosted runner
is that you need to install on it the GitHub Actions Runner software, so it can
be connected with the GitHub Actions service.
...
A job matrix, aka. matrix strategy, lets you use variables in a single job
definition to automatically create multiple job runs. This feature is incredibly
useful when you need to test your builds across multiple versions of a language
runtime, or on different operating systems.
...
To remove specific configurations from the matrix, use
`jobs.<job_id>.strategy.matrix.exclude`. Excluded configurations do not run.

You may need to pass the matrix data into the relevant `setup-<runtime>` action
that prepares the runtime environment. That's possible because the variables
defined in your matrix become properties in the `matrix` context. You can simply
reference these properties in other areas of your workflow file. For instance,
you can use `matrix.version` and `matrix.os` to access the current values of
version and os that the job is using.
...
target runtime environments before publishing a package update. Example for a
Node.js library:
...
Enterprise accounts, organizations owned by enterprise accounts, and
organizations using GitHub Teams, can create and manage additional runner
groups.
...
If you are using GitHub-hosted runners, you MUST use an Ubuntu runner. If you
are using self-hosted runners, you MUST use a Linux machine as your runner, and
Docker MUST be installed on it.
...
[NOTE]
======
You cannot create and use service containers inside a composite action. They are
available only for JavaScript and Docker actions.
======

You can configure jobs in a workflow to run directly on a runner machine or in a
Docker container:

* Run jobs in a container using Docker *bridge* mode; access the container via
  its hostname, eg. `redis`.

* Run jobs on the runner machine using Docker *host* mode; access using
  `localhost:<port>` or `127.0.0.1:<port>`.

Example of usage of the Redis container service. The job specifies the type of
...
In the following example, the job is running on the host image rather than in a
container. We can tell because there is no container specified, and instead
...
* For private repositories, each GitHub account receives some free minutes of
  ...
  subscription plan. Usage beyond the free amounts is capped by spending limits.
