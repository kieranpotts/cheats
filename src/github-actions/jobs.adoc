...
than a string. Expressions can be used in other attributes, which assume string
values by default, using the `${{ <expression> }}` notation.
...
With this configuration, job4 will start after job1 has completed, and job3 will
start after both job1 and job2 have completed.
...
The `needs` and `if` attributes can be combined. In the following example, job3
will run only after job1 and job2, and only if they were both successful.
