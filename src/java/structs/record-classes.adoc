...
Since Java 17, it is much, _much_ easier to create this kind of class using the
`record` keyword. All you need to do is:
...
you could check that `age` is a positive number. If it is not, you could throw
an `IllegalArgumentException`. This is a good use case for a
*compact canonical constructor*. Instead of the following…
