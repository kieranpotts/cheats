...
The Java compiler can infer the types of some variables based on assigned
values. This feature is called *type inference*.

This feature began in Java 8 when lambda expressions were introduced. In lambda
expressions, the type declarations for arguments were optional as the compiler
was designed to infer the argument types from the surrounding context. Example:
...
This feature was extended in Java 10. The extended feature is called
*local variable type inference* and it allows the compiler to infer the type of
a local variable from the value assigned to it. Example:
...
[NOTE]
======
Java permits variables to be named `var`, but this is NOT RECOMMENDED. We SHOULD
treat `var` as a reserved keyword, even though it is not.
