...
supports primitive data types. For example, int, float, etc. are primitive data
types. They are not objects, as they do not have methods.
...
The example below shows how we store a primitive value in an object. This
concept is called *boxing*. This is the old syntax, and it is deprecated – it
will emit an error on build:
