...
the sense that they are accessible from anywhere within the same package (which
is the same default behavior as classes themselves).
...
* `protected` members can be accessed from anywhere within the same package (the
  default behavior), and in addition they can be accessed from any subclass –
  including subclasses defined in other packages.

* `public` members can be accessed from absolutely everywhere.

The following table summarizes the access levels for class members (the default
