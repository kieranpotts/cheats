...
Redis has several commands, via the `redis-cli` utility, that allow you to change the server's configuration on the fly. (The following examples are correct for Redis Server v4 and Ubuntu v18.04). Other Redis interfaces, such as https://github.com/IBM-Cloud/redli[Redli], are available as alternatives to `redis-cli`.
