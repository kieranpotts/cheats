...
Using the AWS Provider, you can generate, store, and retrieve secrets using two
services:
...
Notice the `password` argument. This is referencing a parameter stored in SSM
Parameter Store. Let's create some configuration to generate the password and
...
This next block takes the `result` from the `random_password.main` resource and
...
This next block retrieves the password from SSM Parameter Store. Notice the use
of the `depends_on` argument. This is used to ensure that password is created
...
Now the value of the password can be referenced via the expression
`data.aws_ssm_parameter.rds_password.value`. This is used to set the password in
the `aws_db_instance` resource, above.

We can output the values of the RDS instance using the `output` block. The
`rds_password` output is marked as `sensitive`, which means it will not be
displayed in the console output when running `terraform apply`. This is a good
practice for sensitive information like passwords.
...
AWS RECOMMENDS storing access credentials such as access keys and passwords in
AWS Secrets Manager, rather than the Parameter Store. Secrets Manager is a paid
service, while Parameter Store is free. Secrets Manager provides additional
features such as automatic rotation of secrets, which is not available in
Parameter Store.
...
When you `terraform apply` this, you should see the secret added to the AWS
...
Alternatively, you can output the individual values, by referencing the keys in
the JSON object.
