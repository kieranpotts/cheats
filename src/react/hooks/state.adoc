...
The following component will render a button labelled "Add Item" and a list of
items labelled "Thing 1" and "Thing 2". When the button is clicked, the original
array will be updated with a new item – we can see that my inspecting the
console log – but the new item will not be rendered in the list.
...
(to reduce duplication of code). React doesn't know that it should re-render the
component when the value of `thingsArray` changes.

To make the component reactive, we need to use the `useState` hook. The
`useState` hook is a function that returns an array with two elements: the first
element is the current value of the state variable, and the second element is a
function that we can use to update the state variable.
...
state setter function accepts a callback as its first argument, and that
callback is given the current value of the state variable as input, and it is
expected to return the new value of the state variable.
...
includes the new item. ES6's spread syntax is the most convenient way to achieve
this.
...
When a state value is an object, and we want to update a property of that
object, we should (again) replace the _whole_ object.
...
state, and its behavior differs from the `useState` hook. With `setState`, you
can return a new object that contains only a subset of the properties of the
current state object, and it would merge the new object with the old one, thus
updating only the properties that exist in the new object.
======
...
*State* refers to values that are
_managed by the component itself. State is mutable; it can – indeed, it _should_
– change during the lifecycle of a component. The purpose of using state is to
dynamically re-render the component when its state changes. This is what
...
Child components can trigger mutations in the parent component's state simply by
calling state setter functions that are passed from the parent via the child's
props.
...
have _one source of truth_ for each piece of state. In this case, the `on` value
originates from the parent component, and any changes to those values should be
managed by the parent component and allowed to trickle back down to the child
...
Then the child component would simply call the callback function in the normal
way:
...
This is actually a better pattern. It is the parent component that creates the
`id` values, which it needs for its own internal logic. Ideally that logical
