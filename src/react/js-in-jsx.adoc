...
delimiters for the JavaScript expression, while the inner brackets are for the
JavaScript object literal from which React will compile the string value of the
`style` attribute.
...
nested JavaScript expression resolves to an object, and the expression is nested
within JSX, React will attempt to render the object as JSX. If an array of
objects is returned, React will render each object in the array as JSX.
...
This provides a convenient way to map over an array of data and generate a
