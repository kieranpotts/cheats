...
Even though `React` is not explicitly used in the following example, it will be
once the file is compiled. It is this API that is used to create React
...
From React v17, the explicit import of `React` is no longer required. And the
`ReactDOM` API changed in React v18. So the above code will need to be
...
Then follow the prompts to create a new project. Select "react" as the framework
when prompted.

Vite requires some subtle changes to how you compose React components. For
example, any images referenced in JSX must be imported at the top of the file,
so the bundler knows where to source the images from.
