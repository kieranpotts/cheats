...
There must be a line break at the end of the command, ie. the file must end with
an empty line.

If installing a system-wide crontab – in `/etc/crontab` or `/etc/cron.d`
fragments – then you also need to append the command with a named user that will
run it:
