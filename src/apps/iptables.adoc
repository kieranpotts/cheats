...
An `iptables` firewall blocks incoming and outgoing connections except for the
ones that we explicitly approve, based on rules.
...
The following rules will allow traffic to the following services and ports:
HTTP (80), HTTPS (443), SSH (22), and ping. All other ports will be blocked.
...
Next, you need to ensure that the firewall rules are activated every time you
