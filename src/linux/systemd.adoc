...
Systemd knows to look for `*.service` files for service management commands, so
...
(without restarting), you can issue the reload command to initiate that process:
...
configuration, you can issue the reload-or-restart command. This will reload the
configuration in-place if available. Otherwise, it will restart the service so
...
To remove the symbolic link that indicated that the service should be started
automatically:
