...
Neovim is a modal text editor: keys behave differently depending on the current
*mode*. The main modes are:

* *Normal mode* for navigation and running commands. This is the default mode.
* *Insert mode* for typing text into the buffer.
* *Visual mode* for selecting text.
* *Command-line mode* for running `:` commands.

Source: https://vim.rtorr.com/[vim.rtorr.com].
...
|`Ctrl+d`
|Move cursor and screen down half a page.

|`Ctrl+u`
|Move cursor and screen up half a page.
|===
...
|`Ctrl+p`
|Insert (auto-complete) previous match before the cursor.
...
|`v`
|Start visual mode, mark lines, then do a command (like `y` to yank).
...
|`yw`
|Yank the characters of the word from the cursor position to the start of the next.

|`yiw`
|Yank (copy) word under the cursor.

|`yaw`
|Yank word under the cursor and the space after or before it.
...
|`gp`
|Put (paste) after cursor and leave cursor after the new text.
...
|`dw`
|Delete the characters of the word from the cursor position to start of next.

|`diw`
|Delete (cut) word under the cursor.

|`daw`
|Delete word under the cursor and the space after or before it.
...
|`\vpattern`
|"Very magic" pattern: non-alphanumeric characters are interpreted as special regex.
