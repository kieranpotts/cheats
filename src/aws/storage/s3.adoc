...
S3 is AWS's object storage service. It is enormously scalable and durable. A single bucket can store an *unlimited amount of data* and an unlimited number of objects. Each individual object can be up to 5TB in size, with the largest object that can be uploaded in a single PUT request being 5GB.
...
Like a key-value store, objects are accessed via a key, which is unique to the bucket. Two URL formats are supported for retrieval:
...
All S3 storage classes have eleven 9s *durability*, but *availability* varies depending on how many availability zones the data is stored in.
...
* *S3 Glacier Deep Archive*: Used for long-term archiving of data that you may never need to access again. Retrieval, if you did need it, may take hours, but it is very low cost.

Glacier-specific features include Object Lock and Vault Lock. *Object Lock* stores objects in a write-once-read-many (WORM) state, which prevents the objects from being deleted or overwritten for a fixed time, or indefinitely. *Vault Lock* can also be used to enforce the WORM model, but with the addition that once the policy is enforced you can lock down the policy, preventing future edits. In effect, the whole object store is locked down. These are good features to enforce data retention compliance requirements.
...
Versioning is a means of keeping multiple variants of an object in the same bucket. With versioning enabled, if you upload a new file with the same name as an existing one, the old one will be persisted as an older version of the same object.
