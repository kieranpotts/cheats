...
SSD is more expensive than HDD, for cost-sensitive use cases HDD may be a better option. A hard disk drive is a magnetic disk with moving parts. It is an older technology and much slower than SSD.
...
AWS uses the HTTPS protocol to expose an API to manage buckets and to read and write objects to them. This makes it very easy to write programs that programmatically interact with object storage.
