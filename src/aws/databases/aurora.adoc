...
* *Multi-master*: This is a *preview* feature that will allow the scaling out of
  write operations within a region.

* *Serverless*: On-demand, auto-scaling configuration. Does not support read
  replicas or public IPs. Can access only through VPC or Direct Connect (not a
  VPN). (Serverless means the database is not running all the time; it gets
  instantiated only once it starts receiving traffic.)
****
