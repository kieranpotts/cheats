...
The main settings to pay attention to are `maxretry`, `findtime` and `bantime`. The `maxretry` setting determines how many invalid login attempts can be made from any one IP address before that IP address is blacklisted. The `findtime` setting determines the number of seconds in which `maxretry` attempts are allowed. The `bantime` setting determines the length of time in seconds for which an IP address remains blacklisted; a negative number makes the ban permanent.
