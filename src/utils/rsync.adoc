...
`rsync` (Remote Sync) is an open source command line utility that provides fast,
incremental file transfer from one host to another. It is "incremental" because
it copies only the differences between the source and destination files, ie. what
has changed since the last sync. It is widely used on Unix-based systems.
...
* `-v` = verbose output
* `-r` = copies data recursively (but does not preserve timestamps and permission
...
set up some cron tasks to rsync directories over to a backup server on a
regular basis.
