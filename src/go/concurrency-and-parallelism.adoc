...
* Concurrency: two or more threads are making progress, but both are not running
  at the same time.
...
The `go` keyword starts a new goroutine in a non-blocking way, so the execution
of the main thread immediately continues to the `say("hello")` call. So you will
get a seemingly random output like this:
...
By default, sending or receiving data on a channel is a blocking operation. So
if a Goroutine is waiting to receive something through a channel, it will just
pause and wait until it gets what it needs.
...
This is an example of a worker pool pattern implementation, a common pattern in
Go and concurrent programming in general. A worker pool simply allows you to
execute the same function concurrently.
