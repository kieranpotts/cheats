...
This is done using the `<slot>` element. *A slot allows content outside of the shadow root to be rendered inside the shadow root's DOM tree.* It's similar in concept to Angular's `ng-transclude` directive or React's `children` prop.
