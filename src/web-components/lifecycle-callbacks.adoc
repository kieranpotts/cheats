...
content to the element, to set up event listeners, and otherwise initialize the
component's dynamic behavior in the DOM.
...
typically used for cleanup code: removing observers and event listeners that
...
* In response to an attribute change notification, eg. when an animation
  started.
