...
shadow DOM, and HTML templates. Each of these APIs may be used independently, or
combined (optionally with other web platform APIs) to create custom, reusable
HTML components.
...
not a requirement of creating a custom element; if a custom element is only ever
injected into the DOM using JavaScript, then there is no need to define an HTML
tag name for it.
